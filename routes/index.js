...
exports.test = function(req, res) {
	...
	var address = "Filbert & Taylor Streets (Russian Hill)";
	//var geourl = encodeURIComponent("https://maps.googleapis.com/maps/api/geocode/json?address=" + address + "&sensor=true&key=" + apiKey);
	var options = {
		host: 'maps.googleapis.com',
	    path: "/maps/api/geocode/json?address=" + encodeURIComponent(address) + "&sensor=true&key=" + apiKey,
	    method: 'GET',
	    headers: {
...
exports.getData = function(req, res) {
	...
	console.log(start);
	console.log(num);
	var extractData = function(data, start, num) {
		if(start != undefined) {

	 		data = data.slice(start, start + num - 1);
	 	}
	 	return data;
	}
	if(!_.isEmpty(cachedData)) {
		resData = JSON.parse(cachedData[CACHE_KEY]);
		res.json(extractData(resData, start, num));
	} else {
 		helper.getJSON(options, function(statusCode, jsonObj){
